/*
	...
	TODO:
		...
		For now the basic functionality is there but it requires adjustments to make it feel more natural.
		In particular:
			*It handles all properties (position, scale, skew/rotation) the same way, but most naturally the biggest effect should be on the scale, followed by position, and either very little or no effect on rotation/skew, OR continuous motion from beginning to end with rotation/skew OR PERHAPS a single bounce for rotation/skew despite the number of other bounces. Investigate this.
			
			*Needs to be more modular, so artists can adjust the proportion of energy absorbed by each property. 
			
			*Requires a GUI to do the above. Probably want to be able to adjust interval, maxbounce, effect on scale, effect on position, and effect on rotation/skew.
			
			*Look into making the dampening change on each keyframe in some way other than just linearly. This affects perception of the material property of the bouncing object.
