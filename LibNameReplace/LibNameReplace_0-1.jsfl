...
function findAndRepl(find, repl, hits, phits, imgdir) {
	...
	//phits is the same but for path replacements.
	...
	for (var i in lib.items) {
		...
		//Replace name if matching
			...
			hits = findAndRepl(find, repl, hits, phits, imgdir); //Recurse because JSFL sux
		} else {
			fl.trace("・	" + name);
		} /**/
...
if (find != "" && find != null) {
	...
	if (repl != "" && repl != null) {
		...
		fl.trace("\nReplaced " + findAndRepl(find, repl, 0, 0, imgdir));
	} else {
